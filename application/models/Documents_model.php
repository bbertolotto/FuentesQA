...
public function add_DocumentsImages($data){

    if($this->db->insert('ta_documents_images', $data))
    {
        $result = array('retorno'=> 0, 'descRetorno'=> "Archivo imagen grabado correctamente ..!<br><br>Identificador Nº " . $this->db->insert_id(), 'identificador'=> $this->db->insert_id());
...
public function get_documentsByRut($dataInput){

    if($dataInput["filters"]!=""){

        $filters = explode(",", $dataInput["filters"]);
        $this->db->where_in('typeDocument', $filters);
    }

    $this->db->where('number_rut_client', $dataInput["nroRut"]);
    $query = $this->db->get('ta_documents');
