...
/** Begin::CC032:: **/ 
public function get_offer_product($dataInput){

      $this->db->where('id_campaign', $dataInput["id_campaign"] );
      $this->db->where('id_product', $dataInput["id_product"] );
      $this->db->where('id_channel', $dataInput["id_channel"] );
      $this->db->where('id_new_client', $dataInput["id_new_client"] );
      
      $query = $this->db->get('ta_parameters_offer_product');
      if ($query->num_rows() > 0) {
          return $query->row();
      } else {
          return false;
